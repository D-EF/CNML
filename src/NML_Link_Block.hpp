 ...
 * @LastEditTime: 2024-03-06 18:22:32
...
namespace NML{
    namespace Link_Block{
         ...
         */
        template <typename Value_Type> struct Link_Block_Node {
            /** 上一个节点 */
        ...
        enum Behavior_Pattern__Add_LinkBlock{
            ...
            lazy,
            /** 积极添加, 当前节点无法容纳插入内容时直接分为3块 >> [0,index):before, [index,length):inset_values, [index+length,...]: after*/
            active,
        };
         ...
         */
        template <typename Value_Type> inline bool check_ELE__Link_Block(Link_Block_Node<Value_Type>*& head_node, Link_Block_Node<Value_Type>*& now_node, Link_Block_Node<Value_Type>*& previous_node)
        {
         ...
         * @param _delete_data_item 是否对每个数据元素执行 delete , 默认 false
         */
        template <typename Value_Type> void free_LinkBlock__After(Link_Block_Node<Value_Type>* header_node,bool _delete_data_item=false);
         ...
         * @param value                    添加的内容
         * @param origin_node              访问节点, 应该可以用header节点访问到, 用于重置 0 下标的位置
        ...
        template <typename Value_Type> bool inset_LinkBlock(
            Link_Block_Node<Value_Type>& header_node, Idx index__offset, Value_Type* value, 
            Link_Block_Node<Value_Type>* origin_node=0,
            Idx _length_value=1, Behavior_Pattern__Add_LinkBlock _paternadd=lazy, Idx _add_node_length=__MIN_LINK_BLOCK_SIZE__
        );
         ...
         */
        template <typename Value_Type> Link_Block_Node<Value_Type>* merge_LinkBlock(Link_Block_Node<Value_Type>*& origin_node, Idx node_length, Idx _min_length);
